...
export default function LoginClient() {
  ...
  useEffect(() => {
    ...
    if (urlMessage === 'account_created') {
      setMessage(t("login_account_created", language));
    }
    
    if (authError) {
      setError(t("login_no_account_error", language));
    }
  }, [searchParams]);
    ...
    try {
      ...
      if (result?.error) {
        if (result.error === 'AccessDenied') {
          ...
          setError(t("login_error", language));
        }
      } else if (result?.ok) {
        router.push('/dashboard');
      }
    } catch (err) {
      setError(t("login_unexpected_error", language));
    ...
    <>
      ...
      <p className="text-gray-400 text-sm text-center mt-6">
        ...
        <a href="/signup" className="text-blue-400 hover:text-blue-300 font-medium">
          {t("login_create_account", language)}
        </a>
      </p>
    </>
  );
