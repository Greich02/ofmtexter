...
export async function generateTextWithProvider({ exchanges, tone, objectif, count }) {
  ...
  // Ajout du champ instructions et language dans le body
  const res = await fetch("/api/generate-text", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ exchanges, tone, objectif, count, provider, instructions: arguments[0].instructions, language: arguments[0].language })
  });
